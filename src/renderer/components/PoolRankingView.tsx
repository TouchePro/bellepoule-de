...
const PoolRankingView: React.FC<PoolRankingViewProps> = ({
  ...
  const handleExport = (format: 'csv' | 'xml' | 'pdf') => {
    if (onExport) {
      ...
      showToast(
        t('messages.export_format_not_implemented', { format: format.toUpperCase() }),
        'warning'
      );
    }
  };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      await exportRankingToPDF(
        overallRanking,
        t('poolRanking.export_pdf_title'),
        weapon,
        cols,
        logo,
        rankingTemplate
        ...
        <div>
          ...
          <p className="text-sm text-muted">
            {isInitialRanking
              ...
              : t('poolRanking.subtitle_after_pools', {
                  poolCount: pools.length,
                  count: editedRanking.length,
                })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          {!isInitialRanking && (
            <button
              ...
              title={t('poolRanking.recalculate')}
      ...
      {splitCriteria && splitGroups.length > 0 && (
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
          ...
          {splitGroups.map(g => (
            ...
            >
              {g === Gender.MALE
                ? t('poolRanking.gender_male')
                : g === Gender.FEMALE
                  ? t('poolRanking.gender_female')
                  : g}
            </button>
          ))}
        </div>
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              {poolWinnersOnly && (
                <th style={{ width: '70px' }}>{t('poolRanking.qualif_short')}</th>
              )}
            </tr>
          </thead>
          <tbody>
