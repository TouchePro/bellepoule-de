...
// Fehlermeldungen laufen über REMOTE_ERROR_LABELS (main-Prozess hat kein t(),
// gleiches Muster wie MENU_LABELS/currentMenuLanguage).
const REMOTE_ERROR_LABELS: Record<MenuLang, { serverNotStarted: string; matchNotFound: string }> = {
  fr: { serverNotStarted: 'Serveur distant non démarré', matchNotFound: 'Match introuvable' },
  en: { serverNotStarted: 'Remote server not started', matchNotFound: 'Match not found' },
  de: { serverNotStarted: 'Remote-Server nicht gestartet', matchNotFound: 'Match nicht gefunden' },
  'zh-HK': { serverNotStarted: '遠端伺服器未啟動', matchNotFound: '找不到賽事' },
};
ipcMain.handle('remote:setTeamArenaMatch', async (_, competitionId: string, arenaId: string, matchId: string, isLaserPoints: boolean) => {
  try {
    const errLabels =
      REMOTE_ERROR_LABELS[currentMenuLanguage as MenuLang] ?? REMOTE_ERROR_LABELS.de;
    const entry = remoteServers.get(competitionId);
    if (!entry) return { success: false, error: errLabels.serverNotStarted };
    const ok = entry.server.setTeamArenaMatch(arenaId, matchId, isLaserPoints);
    return ok ? { success: true } : { success: false, error: errLabels.matchNotFound };
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : 'Erreur' };
  }
});
